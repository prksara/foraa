    ...
    try {
      ...
      setReportDetails((prev) => ({
        ...
        extractions: prev.extractions.map((e) =>
          e.id === extId ? { ...e, status: "confirmed" } : e,
        ),
      }));
  ...
  const handleRejectExtraction = async (extId) => {
    try {
      ...
      setReportDetails((prev) => ({
        ...prev,
        extractions: prev.extractions.map((e) =>
          e.id === extId ? { ...e, status: "rejected" } : e,
        ),
      }));
      loadReports(); // update main list status
    } catch (err) {
        ...
        >
          ...
          <div>
            <h1 className="page-header__title">
              {reportDetails?.filename || "Loading..."}
            </h1>
            <p className="page-header__desc">
              Review extracted health insights.
            </p>
          </div>
        </div>
          ...
          >
              ...
              >
                ...
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  {reportDetails.download_url && (
                    <a
                      href={reportDetails.download_url}
                      target="_blank"
                      rel="noreferrer"
                  ...
                  <Button
                    variant="ghost"
                    onClick={() => handleDelete(reportDetails.id)}
                    icon={<Trash2 size={14} />}
                  >
                    Delete
                  </Button>
            ...
            <div>
              ...
              {reportDetails.extractions.length === 0 ? (
                <p>
                  No specific structured data was extracted from this report.
                </p>
              ) : (
                <div
                  style={{
                        ...
                        {ext.status === "pending_review" && (
                          ...
                          >
                            ...
                            <Button
                              variant="outline"
                              icon={<X size={14} />}
                              onClick={() => handleRejectExtraction(ext.id)}
                            >
                              Reject
                            </Button>
                          </div>
  ...
  return (
    <div className="page">
      ...
      <ConfirmModal
        ...
        title="Delete Report"
        message="Are you sure you want to delete this report? The extracted data might still remain in your health profile."
        confirmText="Delete"
        isDestructive={true}
      />
    </div>
